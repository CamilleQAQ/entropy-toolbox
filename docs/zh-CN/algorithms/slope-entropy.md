...
已实现的多尺度方法：**MSlopEn**、**RCMSlopEn** 和 **TSMSlopEn**。
...
MSlopEn 将该估计器应用于标准粗粒化信号。RCMSlopEn 先平均各复合偏移的斜率模式分布，再计算熵。TSMSlopEn 对每个时间移位相位计算该估计器，并平均各相位结果。
...
- `scale`：MSlopEn 的最大标准尺度，或 RCMSlopEn 的最大精细复合尺度。
...
MSlopEn 和 RCMSlopEn 返回 `scale` 个值，TSMSlopEn 返回 `kmax` 个值。数值越大，表示在所选阈值和维数下斜率符号模式分布越均匀。使用不同阈值得到的结果不应视为处于同一测量尺度。
