"""Composite Multiscale Ensemble Fuzzy Dispersion Entropy.

At each scale, CMEFuDE evaluates all composite coarse-graining offsets with
ensemble fuzzy dispersion entropy and averages their values."""
...
def cmefude(
    ...
    if x.ndim > 2 or (x.ndim == 2 and 1 not in x.shape):
        raise ValueError(
            "cmefude: x must be 1-D (MATLAB row convention) or a "
    ...
    if scale < 1:
        raise UnboundLocalError(
            "CMEFuDE: output unassigned (the MATLAB 1:scale loop is "
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        for j in range(1, scale + 1):
            vals = [
                ensfude(coarse_grain(x_flat[jj - 1 :], j), m, nc, tau)[0]
                for jj in range(1, j + 1)
            ]
            # MATLAB mean of the offset column: sequential accumulation
            # seeded with the first element (this preserves -0).
            total = vals[0]
            for value in vals[1:]:
                total = total + value
            out[j - 1] = total / j
